...
class DbSeederPostgres {
    ...
    public static function seedDatabase() {
        try {
            ...
            self::seedItems($pdo);
            self::seedCart($pdo);
            self::seedReceipts($pdo);
            self::seedReceiptItems($pdo);
    ...
    private static function seedUsers($pdo) {
        ...
        foreach ($users as $user) {
            try {
                ...
                echo "✅ Seeded user: {$user['username']}<br>";
            } catch (Exception $e) {
                echo "⚠️ Failed to seed user {$user['username']}: " . $e->getMessage() . "<br>";
            }
        }
    }
    
    private static function seedItems($pdo) {
        echo "Seeding items...<br>";
        
        // Check if items static data file exists
        if (!file_exists('staticDatas/items.staticData.php')) {
            echo "⚠️ Items static data file not found, creating sample data...<br>";
            self::createSampleItems($pdo);
            return;
        }
        ...
        foreach ($items as $item) {
            try {
                ...
                echo "✅ Seeded item: {$item['name']}<br>";
            } catch (Exception $e) {
                echo "⚠️ Failed to seed item {$item['name']}: " . $e->getMessage() . "<br>";
            }
        }
    }
    
    private static function createSampleItems($pdo) {
        ...
        if (empty($userIds)) {
            echo "⚠️ No users found, skipping items seeding<br>";
            return;
        }
        ...
        foreach ($sampleItems as $item) {
            try {
                ...
                echo "✅ Seeded sample item: {$item['name']}<br>";
            } catch (Exception $e) {
                echo "⚠️ Failed to seed sample item {$item['name']}: " . $e->getMessage() . "<br>";
            }
        }
    }
    
    private static function seedCart($pdo) {
        echo "Seeding cart items...<br>";
        
        // Check if cart static data file exists
        if (!file_exists('staticDatas/cart.staticData.php')) {
            echo "⚠️ Cart static data file not found, creating sample cart data...<br>";
            self::createSampleCartItems($pdo);
            return;
        }
        ...
        $insertQuery = "INSERT INTO cart (session_token, user_id, item_id, quantity, price_at_time) 
                       VALUES (?, ?, ?, ?, ?)
                       ON CONFLICT (session_token, item_id) DO NOTHING";
        $stmt = $pdo->prepare($insertQuery);
        ...
        foreach ($cartItems as $cartItem) {
            try {
                ...
                echo "✅ Seeded cart item for session: {$cartItem['session_token']}<br>";
            } catch (Exception $e) {
                echo "⚠️ Failed to seed cart item: " . $e->getMessage() . "<br>";
            }
        }
    }
    
    private static function createSampleCartItems($pdo) {
        ...
        if (empty($userIds) || empty($items)) {
            echo "⚠️ Insufficient data for cart seeding<br>";
            return;
        }
        ...
        foreach ($sampleCartItems as $cartItem) {
            try {
                ...
                echo "✅ Seeded sample cart item<br>";
            } catch (Exception $e) {
                echo "⚠️ Failed to seed sample cart item: " . $e->getMessage() . "<br>";
            }
        }
    }
    
    private static function seedReceipts($pdo) {
        echo "Seeding receipts...<br>";
        
        // Check if receipts static data file exists
        if (!file_exists('staticDatas/receipts.staticData.php')) {
            echo "⚠️ Receipts static data file not found, creating sample receipts...<br>";
            self::createSampleReceipts($pdo);
            return;
        }
        ...
        foreach ($receipts as $receipt) {
            try {
                ...
                echo "✅ Seeded receipt: {$receipt['receipt_number']}<br>";
            } catch (Exception $e) {
                echo "⚠️ Failed to seed receipt {$receipt['receipt_number']}: " . $e->getMessage() . "<br>";
            }
        }
    }
    
    private static function createSampleReceipts($pdo) {
        // Get some user IDs
        $userQuery = "SELECT id FROM users LIMIT 2";
        $userResult = $pdo->query($userQuery);
        $userIds = $userResult->fetchAll(PDO::FETCH_COLUMN);
        ...
        if (empty($userIds)) {
            echo "⚠️ No users found for receipts seeding<br>";
            return;
        }
        ...
        $sampleReceipts = [
            [
                ...
                'total_amount' => 123.45,
                'tax_amount' => 12.35,
                'shipping_address' => self::generateDeliveryAddress(),
        ...
        foreach ($sampleReceipts as $receipt) {
            try {
                ...
                echo "✅ Seeded sample receipt: {$receipt['receipt_number']}<br>";
            } catch (Exception $e) {
                echo "⚠️ Failed to seed sample receipt: " . $e->getMessage() . "<br>";
            }
        }
    }
    
    private static function seedReceiptItems($pdo) {
        echo "Seeding receipt items...<br>";
        
        // Check if receipt items static data file exists
        if (!file_exists('staticDatas/receiptItems.staticData.php')) {
            echo "⚠️ Receipt items static data file not found, creating sample receipt items...<br>";
            self::createSampleReceiptItems($pdo);
            return;
        }
        ...
        foreach ($receiptItems as $receiptItem) {
            try {
                ...
                echo "✅ Seeded receipt item: {$receiptItem['item_name']}<br>";
            } catch (Exception $e) {
                echo "⚠️ Failed to seed receipt item: " . $e->getMessage() . "<br>";
            }
        }
    }
    
    private static function createSampleReceiptItems($pdo) {
        ...
        if (empty($receiptIds) || empty($items)) {
            echo "⚠️ Insufficient data for receipt items seeding<br>";
            return;
        }
        ...
        foreach ($sampleReceiptItems as $receiptItem) {
            try {
                ...
                echo "✅ Seeded sample receipt item: {$receiptItem['item_name']}<br>";
            } catch (Exception $e) {
                echo "⚠️ Failed to seed sample receipt item: " . $e->getMessage() . "<br>";
            }
        }
    }
    ...
    private static function generateDeliveryAddress() {
